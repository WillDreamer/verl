...
class FullyAsyncRollouter(FullyAsyncRayPPOTrainer):
    ...
    def __init__(
        ...
        # Use OmegaConf to safely access config.env (may be blocked by struct mode)
        from omegaconf import OmegaConf
        self.use_env_interaction = False
        self.env_config = None
        
        try:
            # Try to access env config using OmegaConf
            if hasattr(config, "env"):
                # Use OmegaConf.select to safely access nested config
                env_config_raw = OmegaConf.select(config, "env", default=None)
                if env_config_raw is not None:
                    env_name = OmegaConf.select(env_config_raw, "env_name", default=None)
                    if env_name is not None:
                        self.use_env_interaction = True
                        # Store env config as OmegaConf object for later use
                        self.env_config = env_config_raw
        except Exception as e:
            print(f"[FullyAsyncRollouter] Warning: Failed to check env config: {e}")
            # Fallback: try direct access (may fail if struct mode is enabled)
            try:
                if hasattr(config, "env") and config.env is not None:
                    if hasattr(config.env, "env_name") and config.env.env_name is not None:
                        self.use_env_interaction = True
                        self.env_config = config.env
            except Exception as e2:
                print(f"[FullyAsyncRollouter] Warning: Direct access also failed: {e2}")
                self.use_env_interaction = False
        ...
        if self.use_env_interaction:
            # Use placeholder dataset for environment-based training
            # The actual data is generated by the environment during reset()
            from recipe.fully_async_policy.env_dataset import EnvPlaceholderDataset
            
            # Determine dataset size from config or use a default
            train_dataset_size = config.data.get("train_dataset_size", 10000)  # Default size
            val_dataset_size = config.data.get("val_dataset_size", 1000)  # Default size
            
            print(f"[FullyAsyncRollouter] Using EnvPlaceholderDataset for environment-based training")
            print(f"[FullyAsyncRollouter] Train dataset size: {train_dataset_size}, Val dataset size: {val_dataset_size}")
            
            train_dataset = EnvPlaceholderDataset(
                size=train_dataset_size,
                tokenizer=tokenizer,
                config=config.data,
                processor=processor,
            )
            val_dataset = EnvPlaceholderDataset(
    ...
    def _validate(self):
        """
        Override _validate to handle environment interaction mode.
        For env mode, validation is skipped or handled differently since data comes from environment.
        """
        # For environment interaction mode, skip validation or return empty metrics
        if self.use_env_interaction:
            print("[FullyAsyncRollouter] Skipping validation for environment interaction mode")
            print("[FullyAsyncRollouter] Validation will be handled during environment rollout")
            return {}
        
        # For non-env mode, use parent class validation
    ...
    async def _feed_samples(self):
        ...
        # Initialize environment manager for environment interaction mode
        envs = None
        if self.use_env_interaction:
            try:
                ...
                # Use stored env_config instead of accessing self.config.env directly
                env_config = self.env_config
                if env_config is None:
                    # Fallback: try to access config with OmegaConf.select
                    from omegaconf import OmegaConf
                    try:
                        env_config = OmegaConf.select(self.config, "env", default=None)
                        if env_config is None:
                            raise ValueError("env config is None after OmegaConf.select")
                    except Exception as e2:
                        print(f"[FullyAsyncRollouter][Feed] Failed to access env config: {e2}")
                        raise
                # Initialize environment manager in executor (synchronous call)
                envs = await loop.run_in_executor(None, lambda: make_envs(env_config))
                print(f"[FullyAsyncRollouter][Feed] Environment manager initialized: {type(envs).__name__}")
            except Exception as e:
                print(f"[FullyAsyncRollouter][Feed] Failed to initialize environment manager: {e}")
                raise
        ...
        for epoch, batch_dict in continuous_iterator:
            ...
            # For environment interaction mode, reset environment and get initial observation
            if self.use_env_interaction and envs is not None:
                # Extract env_kwargs from full_batch
                env_kwargs = full_batch.non_tensor_batch.get("env_kwargs", None)
                if env_kwargs is not None and len(env_kwargs) > 0:
                    # For single sample, take the first env_kwargs
                ...
                loop = asyncio.get_event_loop()
                obs, infos = await loop.run_in_executor(None, lambda: envs.reset(kwargs=env_kwargs))
                
                # Store initial observation in full_batch for agent loop to use
                # Convert obs to a format that can be stored in non_tensor_batch
                # For single sample, we store the first element of each obs component
                initial_obs = {}
                if obs.get("text") is not None:
                    initial_obs["text"] = obs["text"][0] if isinstance(obs["text"], (list, np.ndarray)) else obs["text"]
                if obs.get("image") is not None:
                    initial_obs["image"] = obs["image"][0] if isinstance(obs["image"], (list, np.ndarray)) else obs["image"]
                if obs.get("anchor") is not None:
                    initial_obs["anchor"] = obs["anchor"][0] if isinstance(obs["anchor"], (list, np.ndarray)) else obs["anchor"]
                
                # Store initial obs and infos in full_batch
                full_batch.non_tensor_batch["initial_obs"] = np.array([initial_obs], dtype=object)
                full_batch.non_tensor_batch["initial_infos"] = np.array([infos[0] if len(infos) > 0 else {}], dtype=object)
                
                print(f"[FullyAsyncRollouter][Feed] Environment reset completed for sample {self.global_steps}")
    ...
    async def _process_single_sample_streaming(self, rollout_sample: RolloutSample):
        ...
        #* 调用异步生成方法，返回生成结果ret和是否取消标志is_cancel
