...
conda create -n verl_agent_webshop python==3.10 -c conda-forge -y
conda activate verl_agent_webshop
...
# apt install -y default-jdk  # 需要sudo权限，改用conda安装Java
...
conda remove -y faiss faiss-cpu faiss-gpu mkl mkl-include intel-openmp || true
conda install -y -c conda-forge "blas=*=openblas" "faiss-cpu>=1.7,<2.0" libgcc-ng libstdcxx-ng
pip install --no-cache-dir --upgrade --force-reinstall pyserini
